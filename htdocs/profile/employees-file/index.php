...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$afm = trim($_POST["afm"]);
	if (empty($afm)) {
		$err = "Παρακαλώ εισάγετε τον ΑΦΜ του εργαζομένου.";
	}

	// Validate name
	$name = trim($_POST["name"]);
	if (empty($name)) {
		$err = "Παρακαλώ εισάγετε το όνομά του εργαζομένου.";
	}

	// Validate surname
	$surname = trim($_POST["surname"]);
	if (empty($surname)) {
		$err = "Παρακαλώ εισάγετε το επώνυμό του εργαζομένου.";
	}

	// Validate contract
	$contract = trim($_POST["contract"]);
	if (empty($contract)) {
		$err = "Παρακαλώ εισάγετε τον τύπο απασχόλησης του εργαζομένου.";
	}

	// Validate role
	$role = trim($_POST["role"]);
	if (empty($role)) {
		$err = "Παρακαλώ εισάγετε την ειδικότητα του εργαζομένου.";
	}

	// Validate email
	$email = trim($_POST["email"]);
	if (empty($afm)) {
		$err = "Παρακαλώ εισάγετε τη διεύθυνση email του εργαζομένου.";
	}
...
<section class="grid">
	...
	<div class="row">
		...
		<section class="c9">
			...
			<p class="space-top">
				Παρακάτω εμφανίζονται οι εργαζόμενοι στην επιχείρισή σας. Μπορείτε να προσθέσετε νέους εργαζομένους, καθώς και διαχειριστείτε τα στοιχεία αδειών, τηλεργασίας κ.α., κάνωντας κλίκ στην σειρά του πίνακα, για το κάθε ατόμο.
			</p>
